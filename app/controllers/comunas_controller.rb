class ComunasController < ApplicationController
  ...
  def create
    ...
    if @comuna.save
      ...
      redirect_to comunas_index_path, notice:"Ocurrio un error al crear la comuna"
  ...
  def update
    ...
    if @comuna.update(comuna_params)
      redirect_to comuna_path(@comuna.id), notice:"Comuna modificada con exito"
  ...
  def destroy
    @comuna = Comuna.find(params[:id])
    @comuna.destroy
    redirect_to comunas_index_path, notice: "Comuna eliminada con exito"
